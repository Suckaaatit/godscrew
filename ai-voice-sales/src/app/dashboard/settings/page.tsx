    ...
    try {
      const response = await fetch("/api/vapi/webhook", {
        ...
        body: JSON.stringify({
          message: {
            type: "status-update",
            call: { id: `test-call-${Date.now()}`, metadata: { prospect_id: "00000000-0000-0000-0000-000000000000" } },
          },
        }),
      });
      const payload = await response.json();
      if (!response.ok || payload.error) throw new Error(payload.error || "Vapi test failed.");
      toast.success("Vapi connection healthy.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Vapi test failed.");
    }
  };
    ...
    <div className="space-y-5">
            ...
            {loading ? (
              ...
              <>
                <div className="flex items-center justify-between rounded-xl border border-[var(--line)] bg-[rgba(255,255,255,0.02)] px-3 py-2">
                  <span className="text-[var(--text-muted)]">Assistant ID</span>
                  <span data-mono="true">{settings?.assistant_id_masked || "-"}</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-[var(--line)] bg-[rgba(255,255,255,0.02)] px-3 py-2">
                  <span className="text-[var(--text-muted)]">Phone Number ID</span>
                  <span data-mono="true">{settings?.phone_number_id_masked || "-"}</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-[var(--line)] bg-[rgba(255,255,255,0.02)] px-3 py-2">
                  <span className="text-[var(--text-muted)]">Status</span>
                  <span className="flex items-center gap-2">
                    {statusDot(Boolean(settings?.integrations.vapi))}
                    {settings?.integrations.vapi ? "Active" : "Inactive"}
                  </span>
                </div>
              </>
            )}
            <Button onClick={testVapi} variant="outline">
              Test Connection
            </Button>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-2 md:grid-cols-4">
          {[
            { label: "Vapi", ok: settings?.integrations.vapi || false },
