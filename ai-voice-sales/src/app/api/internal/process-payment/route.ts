 ...
 *
 * Background worker called via self-call pattern from /api/vapi/actions.
 ...
 * 3. Send payment link email via Resend
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (emailResult.error) {
      ...
      // Continue — payment record should still be created so dead-letter can retry
    } else {
