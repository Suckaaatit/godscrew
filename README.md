...
The system uses Vapi as voice orchestrator (Deepgram STT → Groq Llama 3.1 70B → Cartesia TTS) achieving ~170ms end-to-end latency. Next.js API routes on Vercel handle all webhooks and tool calls. Every tool call returns an immediate response to Vapi (<500ms) while heavy work (Stripe sessions, email sending) fires via self-call endpoints to separate serverless execution contexts. The dead-letter cron catches any failed background tasks every 5 minutes. Upserts and atomic row locking handle every race condition, retry, and out-of-order webhook.

**Stack:** Next.js App Router → Vercel Pro | Supabase Postgres | Stripe Checkout Sessions | Resend Email | Vapi Voice AI | Zod Validation
...
│   │   └── api/
│   │       ├── vapi/
│   │       │   ├── webhook/route.ts         ← Call lifecycle (end-of-call)
...
#    Vapi:   Assistant → serverUrl = https://your-app.vercel.app/api/vapi/webhook
#            Tool server URLs = https://your-app.vercel.app/api/vapi/actions
...
# 8. Configure Vapi assistant (run once after deploy)
curl -X PATCH "https://api.vapi.ai/assistant/YOUR_ASSISTANT_ID" \
  -H "Authorization: Bearer YOUR_VAPI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ ... }'  # See Vapi Assistant Configuration in spec doc
```
...
```bash
# Test Vapi webhook (simulate end-of-call)
curl -s -X POST https://your-app.vercel.app/api/vapi/webhook \
  -H "Content-Type: application/json" \
  -d '{"message":{"type":"status-update","call":{"id":"test-call-123","metadata":{"prospect_id":"00000000-0000-0000-0000-000000000000"}}}}' | jq .

# Test Vapi actions (simulate tool call)
curl -s -X POST https://your-app.vercel.app/api/vapi/actions \
  -H "Content-Type: application/json" \
  -d '{"message":{"type":"tool-calls","call":{"id":"test-call-123","metadata":{"prospect_id":"00000000-0000-0000-0000-000000000000"}},"toolCallList":[{"id":"tc-test-1","type":"function","function":{"name":"confirm_payment","arguments":"{}"}}]}}' | jq .
...
### Problem 1: Vapi tool call times out (dead air)
...
- [ ] ✅ `curl /api/vapi/webhook` = returns `{"ok":true}`
...
- [ ] ✅ Vapi assistant configured with deployed serverUrl + tool URLs
- [ ] ✅ Phone numbers inserted into `phone_numbers` table
- [ ] ✅ Error tracking = Vercel logs dashboard active
- [ ] ✅ All webhook endpoints responding (Vapi, Stripe, Resend)
- [ ] ✅ Test call from Vapi dashboard → your personal phone → data in Supabase
